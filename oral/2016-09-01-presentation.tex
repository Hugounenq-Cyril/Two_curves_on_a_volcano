...
\textbf{Problem:} if height $> 0$ then $\ker(\pi-\lambda)$ is not a cyclic group.
Thus we have to use all different subgroups of $\ker(\pi-\lambda)$, which is of size $\ell^{h+k}$ compare to $\ell^{k}$ in the other cases.
\end{frame}%>>>
