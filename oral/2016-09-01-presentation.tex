...
Let $\mathbb{F}_q\subset \mathbb{K}$, and let $m \in \mathbb{N}$, we denote by
\begin{itemize}
\item $E(\mathbb{K})[m]= \{ P \in E(\mathbb{K}) , mP=0_E \} $
...
Separable isogenies of degree $\ell$ are represented by rational maps
%\[
...
\begin{frame}
\frametitle{Representation of isogenies}
...
\item For each $a \in \left( \mathbb{Z}/p^k\mathbb{Z}\right)^{\times}$:
 \begin{enumerate}
...
\item $E[p^k]\simeq\left(\mathbb{Z}/p^{k} \mathbb{Z} \right) \quad \quad \quad \quad$  with $p^{k}>4r$
\item $E[\ell^k]\simeq\left(\mathbb{Z}/\ell^{k} \mathbb{Z} \right) \times \left(\mathbb{Z}/\ell^{k} \mathbb{Z} \right)$ with $\ell^{2k}>4r$
\end{itemize}}
Since we want to map $E[\ell^k]$ on $E'[\ell^k]$ we have to repeat the interpolation step $O(\ell^{2k} \times \ell^{2k})=O(r^2)$ in average, it leads to a too high complexity. 
\pause

$\Rightarrow$ We can't use anymore the fact that $\phi$ send $E[\ell^k]$ on $E'[\ell^k]$, those groups are too big, we have to do more restrictions:
\begin{itemize}
\item[$\rightarrow$]Therefore we study the $\ell$-torsion group and the action of the Frobenius endomorphism on it.
\end{itemize}
%preciser ce therefore du coup..
\end{frame}

\begin{frame}
%Diapo sur le Frobenius....
%We want to study the action of the Frobenius on the $\ell$ torsion:
One natural intuition is to focus our attention on the eigenvectors for the Frobenius endomorphism.

\begin{rem}
Eigenvector for the Frobenius endomorphism are preserved by $\phi$.
\end{rem}
$\Rightarrow$ We will work with them to manipulate smaller sets for the interpolation.

We denote by $T_{\ell}(E)$ the Tate module of the elliptic curve $E$.%, we study the action of the Frobenius endormorphism $\pi$ on it.
%\begin{prop}
...
\pause
Our goal is to specify a part of the Tate module $T_{\ell}(E)$:
\[
...
%\bluebox{Practice}
%{
%In practice we compute points of the elliptic curves who are of order $\ell^k$ 
%}
...
\end{itemize}}
then through those points \only<1-1>{$\color{red}P_1\color{black},\color{blue}Q_1$}\only<2-2>{$\color{red}P_2\color{black},\color{blue}Q_2$}\only<3-3>{$\color{red}P_3\color{black},\color{blue}Q_3$} we have determined a part of $T_{\ell}(E)$:
\[
...
\begin{frame}
\bluebox{}{
Summarizing, our algorithm for two curves on a cyclic crater of a volcano of $\ell$-isogenies of \textbf{height \boldmath$=0$\unboldmath}  works as follows:
\begin{enumerate}
\item Fix $k$ such that  $\ell^{2k}>4r$
\item Compute diagonal bases $(\color{red}P\color{black},\color{blue}Q\color{black})$ of
...
\]
We get for \only<1-1>{ a such }\only<1-1>{ representation }\only<2-2>{another representation}  of $T_{\ell}(E_0) \bmod \ell^3$:
\begin{columns}
\begin{column}{5cm}
...
\only<2-2>
{
By pushing $P$ by $\color{blue}\psi$ we get the point $\psi(P)$ such that $\color{red} \ell \psi(P)$ is a \emph{horizontal} point of order $\ell^2$.
...
On \only<1-1>{$E_0$}\only<2-3>{$E_1$}\only<4-4>{$E_2$} we have determined this part of \only<1-1>{$T_{\ell}(E_0)$}\only<2-3>{$T_{\ell}(E_1)$}\only<4-4>{$T_{\ell}(E_2)$}:
\[
