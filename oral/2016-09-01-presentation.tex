...
\item For each $a \in \left( \mathbb{Z}/p^k\mathbb{Z}\right)^{\times}$:\hfill $O(r)$
 \begin{enumerate}
  ...
  \end{enumerate}
\end{enumerate}
Complexity polynomial in $p$. \bluebox{Goal:} {Replace $E[p^k]$ by \boldmath $E[\ell^k]$ \unboldmath for a small $\ell$.}
\end{frame}
...
%\begin{enumerate}
Our goal is to compute an isogeny cycle, differently from [Couveignes-Morain '94].
...
\ENSURE $\langle \red P, \blu Q \rangle$ an $\ell^k$  horizontal basis.
\end{algorithmic}
