...
[Bostan,~ Morain,~ Salvy,~ Schost '08] work only for $r < 2p$ using $\tilde{O}(r^3)$ operations in $\mathbb{F}_q$.
...
$\ell$-isogeny & $\Leftrightarrow$ &  Subgroup of size $\ell$  \pause \\
...
\[ (X-\red \lambda)(X- \blu \mu ) \quad \textit{with} \quad \red \lambda \neq \blu \mu \bmod \ell \]
\begin{tabular}{p{10em}p{3em}p{10em}}
Two rational $\ell$-isogenies & $\Leftarrow$  & Two eigenspaces \\ 
& & $\ker( \pi- \red \lambda), \ker( \pi- \blu \mu) $ \\
\end{tabular}
...
\item Diagonalize $\to$ ${\color{red}{T_\lambda(E)}}\oplus\color{blue}{T_\mu(E)} \color{black} \bmod \ell^k$ and ${\color{red}{T_\lambda(E')}}\oplus\color{blue}{T_\mu(E')} \color{black}\bmod \ell^k$
