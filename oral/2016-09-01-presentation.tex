...
We compute $\ell^3$-isogenies of kernel $\ker(\pi-\blu \mu)$
\begin{columns}
\begin{column}{5cm}
...
$\ker(\pi- \blu \mu)$ of size $\ell^3$ is a cyclic group.
\end{column}
\begin{column}{5cm}
...
\textbf{Problem:} if height $> 0$ then $\ker(\pi- \blu \mu)$ of size $\ell^{3+h}$ is not a cyclic group.
...
\textbf{Goal:} compute the kernel of $\ell^k$ horizontal isogenies (codomain curves lying on the crater).
...
\begin{frame}
  ...
  \item We work on $\mathbb{F}_q$ such that $E[\ell] \subset E(\mathbb{F}_q)$,
	...
	\item Then we use $\ell$-adic extensions using constructions from [De~Feo,~Doliskani, Schost '13], [Doliskani,~Shcost '15] where in particular we have a fast computation of the Frobenius.
  \end{itemize}
