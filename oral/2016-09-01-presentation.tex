...
\pause
Our goal is to determine the Tate module $T_{\ell}(E)$ at a fixed precision:
\[
...
When we have diagonalized the Frobenius endormorphism on the Tate Module $T_{\ell}(E_0)\bmod \ell^3$:
\[
\left( \begin{matrix}
\lambda & 0 \\
0 & \mu 
...
\end{matrix} \right) \bmod \textit{\boldmath  $\ell^3$ \unboldmath } 
\]
then we have determined here a decomposition of $T_{\ell}(E_0) \boldmath \bmod \ell \unboldmath $ :
\[
