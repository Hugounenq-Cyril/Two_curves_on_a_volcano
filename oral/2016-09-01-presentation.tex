...
\begin{itemize}
\item  $\mathsf{End}(E)$ is isomorphic to an order $\mathcal{O}$ in a quadratic imaginary number field $K=\mathbb{Q}[\sqrt{t_{\pi}^2-4q}]$.
...
\begin{frame}
  ...
  \item We work on $\mathbb{F}_q$ such that $E[\ell] \subset E(\mathbb{F}_q)$,
	...
	\item then we use $\ell$-adic extensions using constructions from [De~Feo,~Doliskani, Schost '13], [Doliskani,~Schost '15] where in particular we have a fast computation of the Frobenius.
  \end{itemize}
  ...
  \item In practice one half of all $\ell$ are expected to be Elkies primes.
  \item In theory we have a bound $\ell \leq O(\log(q))$ [Shparlinski, Sutherland '14].
  \end{itemize}
  }
\end{frame}
