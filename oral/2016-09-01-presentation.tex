...
\logo{\includegraphics[height=8mm]{uvsq-logo-cmjn.jpg}} 
...
%\title[Structure of $\ell$-isogeny volcanoes applied to Couveignes's algorithm]{Structure of $\ell$-isogeny volcanoes applied to Couveignes's algorithm}
...
\begin{frame}
\titlepage
\includegraphics[scale=0.1]{digiteo.jpg} 
\end{frame}
%\maketitle
%faire un plan
...
\item Endomorphism ring  of elliptic curves %according to Kohel's work in 1996 \cite{Kohel96},
...
%\begin{exe}
%For example we can represent the isogeny between $E_1$ a curve of $j$-invariant $28$ defined over $\mathbb{F}_{101}$ and $E_2$ a curve of $j$-invariant $65$ and cardinality $96$ by the following rational map:
%\[
%\left( x ,y \right) \mapsto \frac{x^3+100x^2+78x+67}{(x+50)^2}
%\]
%\end{exe}
...
\begin{frame}
\frametitle{Motivation}
We focus on one specific problem in the Schoof-Elkies-Atkin (counting point algorithm) context:
\orangebox{Explicit isogeny computation problem}
{Let $E$, $E'$ defined on $\mathbb{F}_q$ be two $r$-isogenous curves: compute the $r$-isogeny that links them.}
\end{frame}
...
{Let $E$, $E'$ defined on $\mathbb{F}_q$ be two $r$-isogenous curves: compute the $r$-isogeny that links them.}
\begin{itemize}
\item \ [BMSS '08] and [CCR '91] work only for $r \ll p$ in $O(M(r)\log(r))$ operations in $\mathbb{F}_q$
\item \ [Couveignes '96] work in $O(r^2)$ operations in $\mathbb{F}_q$ but exponential in $\log(p)$
\item \ [LS '08] works for every $p$ in $O(r^2)$ operations in $\mathbb{F}_q$
\end{itemize}
\begin{itemize}

\item[$\rightarrow$] we focus on the medium charactristic case,

\item[$\rightarrow$] we want to modify Couveignes'algorithm to have an algorithm who works with complexity in $O(r^2)$ but with no exponential dependancy in $\log(p)$
...
$\mathrm{End}(E)=\left( \{ \mathrm{isogenies \  } \phi : E \rightarrow E \} ,+ ,\circ  \right)$ is a ring. %with the addition law and composition law.
\end{defi}
...
\only<2-3> {descending isogeny,}
%\item <2->  $\ell=[\mathcal{O} : \mathcal{O}']$ we say then that $\phi$ is a descending isogeny,
\pause
\item $\ell=[\mathcal{O}':\mathcal{O}]$ we say then that $\phi$ is an \only<2-2> {$\textbf{ascending}$  isogeny,}
...
\bluebox{Couveignes's algorithm}{ %\cite{DBLP:conf/ants/Couveignes96}}{
\begin{algorithmic}[1]
...
\begin{enumerate}
\begin{enumerate}
\item Compute bases $(P)$ of
  $E[p^k]$ and $(P')$ of $E'[p^k]$ with $p^k>4r$;
...
We want to modify this algorithm and work with \boldmath $E[\ell^k]$ \unboldmath instead of $E[p^k]$.
\end{frame}
...
\begin{column}[l]{4cm}
\bluebox{Couveignes'algorithm}
{
$E[p^k]= \left(\mathbb{Z}/p^k \mathbb{Z} \right)$ thus for two basis:
\begin{itemize}
\item $\langle P \rangle=E[p^k]$ 
\item $\langle P' \rangle=E'[p^k]$
\end{itemize} 
 we have to test $O(p^k)=O(r)$ mapping candidates before finding the good one for the interpolation.
}
\end{column}
...
\pause
\item[$\Rightarrow$] thus we want to specify one type of points %more \textbf{restrictive} and \textbf{invariant} under the action of $r$-isogeny 
to limit our number of mapping candidates to $O(\ell^{\frac{2k}{2}})=O(r)$.

\end{itemize}}
\end{column}
\end{columns}
Therefore we study the volcano of $\ell$-isogenies and the action of the Frobenius endomorphism.
\end{frame}
...
\begin{defi}
An $\ell^k$-isogeny is said to be horizontal if it is the composition of $k$ horizontal $\ell$-isogeny.
\end{defi}
...
\[((\color{red}P_1 \color{black}, \color{blue}Q_1 \color{black}),(P_2,Q_2),(P_3,Q_3), (P_4,Q_4), \cdots )\]
}
\only<2-2>{
\[
\left( \begin{matrix}
...
\]
then we have determined a part of $T_{\ell}(E)$:
\[(\color{red!30}(P_1, \color{blue!30}Q_1 ),\color{black}(\color{red}P_2\color{black},\color{blue}Q_2\color{black}),(P_3,Q_3), (P_4,Q_4), \cdots )\]
}
...
\]
then we have determined a part of $T_{\ell}(E)$:
\[(\color{red!30}(P_1, \color{blue!30}Q_1 ),(\color{red!30}P_2,\color{blue!30}Q_2),\color{black}(\color{red}P_3\color{black},\color{blue}Q_3\color{black}), (P_4,Q_4), \cdots )\]
}
\end{exe}
...
\begin{scope}[yshift=10cm]
	\begin{scope}[xshift=4.3cm]
		...
		\draw [line width=1.5pt,red,<-] (A.center) to[bend right=20] (C.center);
			...
			\coordinate (CB) at (275:7.5);
			\draw (C)--(CA);
			\draw [line width=1.5pt,black,->] (C)--(CB);
...
\begin{scope}[yshift=10cm]
	\begin{scope}[xshift=4.3cm]
		...
		\draw [line width=1.5pt,red,<-] (A.center) to[bend right=20] (C.center);
			...
			\coordinate (CA) at (265:7.5);
			\coordinate (CB) at (275:7.5);
			\draw [line width=1.5pt,black,->] (C)--(CA);
			\draw (C)--(CB);
			\draw [line width=1.5pt,black,->] (A)--(C);
			\draw (A) node[fill=white]{$65$};
			\draw (C) node[fill=white]{$60$};
...
\begin{column}[l]{5cm}
\orangebox{}{
On $E$ of $j$-invariant \boldmath $28$ \unboldmath , we have determined this part of $T_{\ell}(E)$:
\[
\[((\color{red}P_1 \color{black}, \color{blue}Q_1 \color{black}),(P_2,Q_2),(P_3,Q_3), \cdots )
\]}
\end{column}
\begin{column}[r]{7cm}
\begin{figure}
...
\begin{column}[l]{5cm}
\orangebox{}{
On $E$ of $j$-invariant \boldmath $65$ \unboldmath, we have determined this part of $T_{\ell}(E)$:
\[
\[((\color{red}P_1 \color{black}, Q_1 \color{black}),(\color{red}P_2 \color{black},Q_2),(P_3,Q_3), \cdots )
\]}
\end{column}
\begin{column}[r]{7cm}
\begin{figure}
...
%\begin{itemize}
%
%\item As in Couveignes's algorithm we need to determine the image of a number $N$ of points such that: $ N>4 r$
%
...
\item Compute horizontal bases $(P,Q)$ of
  $E[\ell^k]$ and $(P',Q')$ of $E'[\ell^k]$ with $\ell^{2k}>4r$;
...
\end{smallmatrix}\right )$ in $(\mathbb{Z}/\ell^k \mathbb{Z})^{2 \times 2}$:
  \begin{enumerate}
  \item\label{alg:ours:interp} compute the interpolation polynomial
    $L_{a,b}$ of degree $\frac{\ell{2k}-1}{2}$ such that
...
\bluebox{}{
For curves which are not on a cyclic crater of a volcano of $\ell$-isogeny
\begin{enumerate}
\item we have to find a suitable $\ell$ such that the volcano of $\ell$ isogeny has a cyclic crater
\item find curves on the crater
\end{enumerate}

$\Rightarrow$ To address those $2$ points we have to use algorithms like the one of Fouquet-Morain[2001]. %\cite{FouquetMorain02}. 
}
Once we have done this we could reduct the problem to the case we have studied before.
\end{frame}
...
\begin{itemize}
\item We have seen a way to determine horizontal basis of the $\ell^k$ torsion through the structure of voclanoes with cyclic crater and the use of the Frobenius. 

\item With thoses basis we have less trials to do for the interpolation. 
...
\end{itemize}}
Pre-print available on : \url{http://arxiv.org/abs/1603.00711}
