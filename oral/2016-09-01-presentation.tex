...
\begin{enumerate}
\item Select least $k$ such that  $\ell^{2k}>4r$
\item Compute $\langle \red P , \blu Q \rangle = \color{red}{E[\ell^k]_{{\lambda}}} \color{black} \oplus \color{blue}{E[\ell^k]_{{\mu}}} $ and $\langle \red P' , \blu Q' \rangle = \color{red}{E'[\ell^k]_{{\lambda}}} \color{black} \oplus \color{blue}{E'[\ell^k]_{{\mu}}} $
...
Since for $i \leqslant h$ and any $P \in  E[\ell^i]$ we have:
\[
\pi(P)=\red \lambda P = \blu \mu P
\]
we cannot immediately distinguish kernels of isogenies of direction $\red \lambda$ from direction $\blu \mu$.
Thus we have to consider $E[\ell^{h+1}]$ to get $P \in \red{ E[\ell^{h+1}]_{ \lambda}}$ such that:
\[
\pi(P)=\red \lambda P \neq \blu \mu P
\]
...
%\pause
We assume that $E$ lies on the crater.
...
$\ker(\pi- \blu \mu)$ of size $\ell^k$ is a cyclic group.
\end{column}
\begin{column}{5cm}
...
\textbf{Problem:} if height $> 0$ then $\ker(\pi- \blu \mu)$ of size $\ell^{k+h}$ is not a cyclic group.
Thus we have to use all different subgroups of $\ker(\pi-\blu \mu)$ of size $\ell^k$.
...
\begin{frame}
\frametitle{Finding cyclic subgroups of $E[\ell^k]$}
We compute the kernel of $\ell^k$ horizontal isogenies (codomain curves lying on the crater).


To this aim we proceed step by step by walking along the volcano crater according to the directions $\red \lambda, \blu \mu$.
\begin{center}
...
\begin{frame}
  ...
  \item We work on $\mathbb{F}_q$ such that $E[\ell] \subset E(\mathbb{F}_q)$,
	...
	\item Then we use $\ell$-adic extensions using constructions from [De~Feo,~Doliskani, Schost '13], [Doliskani,~Shcost '15] where in particular we have a low-cost Frobenius.
  \end{itemize}
  ...
  \item In practice $\frac{1}{2}$ $\ell$ are expected to be "Elkies prime",
  \item In theory we have a bound $O(\log(q))$ [Shparlinski, Sutherland '14].
  \end{itemize}
  }
\end{frame}
