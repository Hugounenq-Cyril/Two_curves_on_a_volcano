...
\begin{prop}
For $\lambda_1 , \lambda_2$ the $2$ eigenvalues of the Frobenius and $k>\nu_{\ell}(\lambda_1-\lambda_2)$, the matrix of the Frobenius action on the $\ell^k$ torsion is only diagonalisable on a cyclic crater of a volcano of $\ell$ isogeny.
\end{prop}

\begin{prop} %EN COURS DE REFORMULATION DE LA PROP PRECEDENTE
For $\lambda_1 , \lambda_2$ the $2$ eigenvalues of the Frobenius, those values are defined in the endomorphism rings who are located at the top of a cyclic volcano of $\ell$ isogeny.
\end{prop}
