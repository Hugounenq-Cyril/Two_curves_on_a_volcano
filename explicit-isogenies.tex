...
Further assume that~$F_1 = \F_q$. Then:
\begin{enumerate}
...
there are exactly $ℓ^{\min(α+i, k) + \min (β+i, k)}$ such points,
and therefore $ℓ^{\min(α, k-i) + \min(β, k-i)}$ such orbits.
