...
\todo{check} and a positive integer $r$ not
divisible by $p$, and returns, if it exists, the kernel of an
$r$-isogeny $ψ:E\to E'$.\todo{Aren't we taking only the $j$-invariants as input?} It is based on the observation that the
...
\todo{In this section, we do \dots blablabla \dots}
...
curves. \todo{over finite fields; should we say it again?}
...
\begin{prop}\label{prop:interpol}
  ...
  $O\bigl(\MM(t)\log(s) + nℓ\bigl(\MM(ℓ^2t) +
...
using~\cite[Chapter~14.5]{vzGG}. More efficient methods for special
...
\begin{proof}
  ...
  is contained in $E(F_n)$ with $n=k-β$.
  ...
  and~$L_{a,b}$ at a cost of~$O(\MM(r)\log^2(r) + \log(r)\log(q))$.
