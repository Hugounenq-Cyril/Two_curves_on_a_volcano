...
\begin{algorithmic}
\REQUIRE $\langle P,Q \rangle = E^0[2^k]$ such that $\pi(P)=\lambda_1P \pi(Q)=\lambda_2Q$
\ENSURE $P$ a point of $2^k$ torsion of $E^{k-1}$ associated to $\lambda_1$ and $L$ a list of $k-1$ horizontal $2$-isogenies associated to $\lambda_2$ which goes from $E^{k-1}$ to $E^0$.
\STATE $L \gets \emptyset$
\STATE $Q2= 2^{k-1}Q$
\FOR{$i=1$ to $k-1$}
\IF {$\pi(P[0]) \neq \lambda_1P[0] $}
\STATE $P \gets P+2^{k-1-h}Q$
\ENDIF
\STATE $\phi \leftarrow \{ E \rightarrow E/ \langle 2^{k-1}P \rangle \}$
\STATE $Q \leftarrow \phi(Q)$
\STATE $P \leftarrow \phi(P)/2$
\STATE $E \leftarrow E/ \langle 2^{k-1}P \rangle$
...
\begin{prop}
The algorithm \ref{1step} has a computational complexity of $O(k^2M(2^k))$operations in $\mathbb{F}_q$. This computational complexity could be expressed as $O(\log(l)^2M(\sqrt{l}))$.
\end{prop}

\begin{proof}
The computations of $\lambda_1P$ takes us $O(M(2^k))$ operations in $\mathbb{F}_q$ whereas the computation of $2^{k-1}P$ takes $O(M(2^k)k)$ . So the costly part of algorithm \ref{1step} is this operation repeated $k$ times. 
\end{proof}
