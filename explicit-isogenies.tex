...
directions of the two horizontal $ℓ$-isogenies.  By
Proposition~\ref{prop:degree-l-torsion}, there is an
extension~$F/\F_q$ of degree~$d_k ≤ ℓ^k$ such that all the points
of~$E[ℓ^k]$ are rational over~$F$.  Algorithm~\ref{alg:diagonal} works
in this extension~$F$. \todo{We could save a log by not working in $F$
...
\todo{Precise somewhere that $\lambda \neq \mu \bmod \ell^k$}
