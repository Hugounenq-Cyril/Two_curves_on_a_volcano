...
% and be able to change this one
\def\division#1#2{\mathsf{divide}(#1,ℓ^{#2})}
...
\STATE $h:=1, u:=1$
\FOR {$i=1$  to  $k-1$}
\STATE $P' \gets \division{P_i}{}$; $Q' \gets \division{Q_i}{}$.
...
\STATE $P' \gets \division{ϕ_i(P_{i-1})}{}$.
\STATE Write~$π(P') = λ P' + b Q_i$ for~$b ∈ ℤ/ℓℤ$ and
let $P_{i} \gets P' - (b/μ) Q_i$.
\ENDFOR
\RETURN $R = \widehat{ϕ}_1 ∘ … ∘ \widehat{ϕ}_{k-1} (\division{P_{k-1}}{k-1})$.
...
\todo{fin du texte nouveau. Reste à justifier le choix du~$k$...}
...
We write~$Q ← \division{P}{}$ for the computation of a preimage of~$P$
by multiplication by~$ℓ$.
The obvious way to compute this would be to use the $ℓ$-division polynomial.
...
the cost of the $\division{P}{}$ operation, where $P$~has coordinates in~$F_k$,
is therefore $\mathsf{R} (k)$.
