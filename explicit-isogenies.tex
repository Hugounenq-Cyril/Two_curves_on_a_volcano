...
\begin{prop}
  ...
  $d_i$ be the degree of the minimal polynomial of $v_i$, and let
  $d=\sum d_i$. The polynomials
  \begin{itemize}
  \item $T∈\F_q[x]$ of degree $d$ such that $T(v_i)=0$ for all $i$,
    and
  \item $L∈\F_q[x]$ of degree less than $d$ such that $L(v_i)=w_i$ for
    all $i$
  \end{itemize}
  can be computed using
  $O\bigl(\MM(d\log(s)) + snℓ\bigl(\MM(ℓ^{n+1}) +
  \MM(ℓ)\log(q)\bigr)\bigr)$ operations in $\F_q$.
\end{prop}
\begin{proof}
  The polynomial $T$ is simply the product of all the minimal
  polynomials $T_i$. By pushing $(v_i,w_i)$ to the smallest level
  $F_j$ containing them, each $T_i$ is computed as explained
  previously at a cost bounded by
  $O\bigl(nℓ\bigl(\MM(ℓ^{n+1}) + \MM(ℓ)\log(q)\bigr)\bigr)$
  operations. Summing over all $i$, we obtain a bound of
  $O\bigl(snℓ\bigl(\MM(ℓ^{n+1}) + \MM(ℓ)\log(q)\bigr)\bigr)$
  operations. Simultaneously, we compute all the polynomials $L_i$
  such that $L_i(v_i)=w_i$, at the same cost.

  Then we arrange the $T_i$'s into a binary subproduct tree and
  multiply them together. Since the degrees $d_i$ are not necessarily
  ...
  multiplication, we build a binary product tree. If the degrees of
  the internal nodes are denoted by $e_i$, the cost of computing the
  tree is $\sum \MM(e_i)$, which, by virtue of the super-linearity of
  $\MM$, we bound by $\MM(\sum e_i)$. The worst case for the Huffman
  coding is attained when all degrees $d_i$ are equal, then
  $\sum e_i∈O(\sum d_i\log(s))$. Hence, we can bound the cost of
  building the whole tree by $O(\MM(d\log(s)))$, \todo{but since no
    multiplication of degree greater than $d$ occurs, this must be
    $O(\MM(d)\log(s))$?!?!}.
  ...
  the polynomial $L$ at the same cost $O(\MM(d\log(s)))$.
  
  \todo{proposition d'adaptation de preuve du premier paragraphe:
  Let denote by $(v_{1i},w_{1i}),\dots,(v_{si},w_{si})$ be pairs 
  of elements of $F_i \setminus F_{i-1}$. We denote by $s_i$ the 
  number of representatives we have $s_i \leqslant \ell^{k+\beta}$, 
   each $T_i$ is computed as explained previously at a cost bounded by
  $O\bigl(nℓ\bigl(\MM(ℓ^{i+1}) + \MM(ℓ)\log(q)\bigr)\bigr)$
  operations. Summing for $i=k - \alpha$ to $k - \beta$, we obtain a bound 
  of  $O\bigl(nℓ\bigl(\MM(ℓ^{2k+1}) + \MM(ℓ)\log(q)\bigr)\bigr)$
  }
\end{proof}
...
Let~$ψ: E→ E'$ be an isogeny between curves with maximal endomorphism rings.
