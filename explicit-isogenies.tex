...
\ENDFOR
\RETURN $R = \widehat{ϕ}_1 ∘ … ∘ \widehat{ϕ}_{k-1} ( ℓ^{h-k+1} P_{k-1} )$. 
...
\begin{lem}\label{lemma:interpolation:minpoly}
  The cost of computing $T^{(n)}=T$ is of
...
The cost of computing all the $R_{u,v}=(R_{u,v,c,d}\ \mid (c,d) \in
C_{u,v})$ is of $O(\M(\ell^{2k})\log(\ell^k))$.
\end{prop}
...
The cost of computing the polynomials $(T,L)$ is of $O(\mathsf{M}(\ell^{2k})\log(\ell^{k}))$ with precomputations of $O(\M(\ell^{2k})\log(\ell^k))$ in $\mathbb{F}_q$
\end{prop}
