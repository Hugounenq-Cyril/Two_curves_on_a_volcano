...
$h$ and find a curve $E_{\max}$ on the crater at the cost of $O(ℓh²)$
factorizations of the $ℓ$-th modular polynomial $Φ_ℓ$. The polynomial
$Φ_ℓ$ is computed using $\tildO(ℓ³\logℓ)$ binary operations and
$O(\MM(ℓ²)\log q)$ operations in $\F_q$, then each factorization costs
...
\begin{proof}
  ...
  $O(k\MM(ℓ^{2k})\log(ℓ^k)) ⊂ O(\MM(r)\log²(r))$ to compute all such representatives.
  ...
  and~$L_{a,b}$ at a cost of~$O(\MM(r)\log²(r) + \log(r)\log(q))$.
