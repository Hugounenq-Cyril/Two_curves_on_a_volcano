...
are many probabilistic algorithms to compute $P_1$ in expected time polynomial
...
\FOR {$i=1$  to  $k$}
\STATE $P' ← \sfdiv(\ell, P_{i-1})$; $Q' ← \sfdiv (\ell, Q_{i-1})$.
\STATE\label{alg:diagonal:frobenius} compute $\pi|(P',Q')=\left( \begin{smallmatrix}
λ + a\ell^{i-1} & b\ell^{i-1}\\
c\ell^{i-1} & μ + d\ell^{i-1} \end{smallmatrix} \right) \pmod {\ell^{i}}$
\STATE{\textbf{if} $b = 0$ \textbf{then} $x ← 0$;
solve equation~$c + (d-a + μ-λ) y = 0$; }
\STATE{\textbf{else} solve equation
  $c x^2 + (d-a+ μ-λ) x - b = 0$;
  $y ← -cx/b$; \textbf{end if}}
\STATE{$P_i ← P' + y Q'$; $Q_i ← x P' + Q'$}
...
\begin{prop}
  ...
  of subsection~\ref{sub:final-interp}
...
%   $O(\MM(\sqrt{r})\log(r)(ℓ^2 + \log(rq)))$ if $ℓ=2$, or by
  ...
  ℓ^2\MM(\sqrt{r} ℓ)\log(r))$.% otherwise.
...
In particular, since in the median case we have~$ℓ = O(1)$,
...
\caption{Left: comparison of horizontal basis computation and
  ...
  increasing $r$. Right: Comparison of the interpolation phase for
  $\F_{101}$, $\F_{2^{62}+169}$ and $\F_{2^{252}+421}$, and increasing
...
$E→E$, for increasing $r$.  The torsion levels involved in the
computations varied from $2^3$ to $2^8$.  Figure~\ref{fig:benchs}
(left) shows the running time for the computation of the horizontal
basis of $E[ℓ^k]$, and that for one execution of the interpolation
step. Total running times are multiplied by an average factor of $r$,
as expected. The staircase behavior of our algorithm is apparent from
the plot. We remark that, though asymptotically negligible, the
horizontal basis computation step dominates one interpolation step for
the tested parameters. Since the sum of all interpolation steps is
asymptotically dominating we focus on this step to compare the running
...
constant, larger primes of cryptographic size are $~30$ times slower.
