...
Let $V$ be a point of order $\ell^{h+j}$ such that $\pi(V)=\lambda_2V$ and $\ell^{h}V=Q$, with $Q$ a generator of a $\ell^j$ horizontal isogeny. We consider: $R$ a $\ell$ division point of $Q$ and $\phi$ the $\ell$ isogeny generated by $\ell^{h}P$ with $P$ a point of order $\ell^{h+1}$ such that $\pi(P)=\lambda_1P$.
Let $W$ be a point of order $\ell^{h+j+1}$ such that $\ell^hW=\phi(R)$, let $T$ be a point such that $\phi(T)=W$. From $\ell^h\phi(T)=\phi(R)$ we deduce that $\ell^{h+1}T=Q^i$ and that $T$ is of order $\ell^{j+h+1}$. Let denote $\widehat{\phi}$ the dual isogeny of $\phi$, then we have $\ell^{h}\widehat{\phi(W)}=\widehat{\phi}(\ell^hW)=\widehat{\phi}(\phi(R))=Q$, thus by hypothesis we have $\pi(\widehat{\phi}(W))=\lambda_2\widehat{\phi}(W)$. Moreover we have $\widehat{\phi} (\phi(T))=\widehat{\phi}(W)$ this implies that $\pi(\ell T)=\pi(\widehat{\phi}(W))=\lambda_2\widehat{\phi}(W)=\lambda_2(\ell T)$. Thus we have for $T$: $\pi(T)=\lambda_2 T + S$ with a $\ell$ torsion point $S$ associated to the "eigenspace" of $\lambda_1$. Then with the action of $\phi$ associated to $\lambda_1$ we have $\pi(\phi(T))=\pi(W)=\lambda_2\phi(T)=\lambda_2W$. 
\item[$\Rightarrow$]
Recursive step: we consider that the property is true to the rank $j>1$, then we will have to prove it for $j+1$.
