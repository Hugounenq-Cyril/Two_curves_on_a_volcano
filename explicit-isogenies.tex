...
the asymptotic complexity of our algorithm is simply~$O(r \MM(r) \log(r))$.
...
Sutherland~\cite{shparlinski2014distribution},
for all primes $q$ and curves $E/\F_q$
and for~$L > \log(q)^ε$ for any~$ε > 0$,
asymptotically half of the primes in the interval $[1,L[$ are Elkies' primes.
Even discarding those Elkies primes for which the height
...
\centering
\includegraphics[scale=1]{benchmarks/graphe-101}
...
\begin{figure}%\label{fig:p=101-149-269}
\centering
...
The torsion levels involved in the computations varied from $2^3$ to $2^7$.
...
to compare the running time for different base fields. Figure ~\ref{fig:p=101-149-269}
shows that the running time does not have an exponential dependency in 
the characteristic.
