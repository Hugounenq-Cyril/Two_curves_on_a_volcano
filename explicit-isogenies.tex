...
\section{Shape of the volcano and path to the crater}
\todo{Eventually explicit the complexity of computing a shape of the volcano i.e. $\mathcal{F}(\ell)$}
  For the input curves we have to determine if the curves are on a volcano with cyclic crater. To do this we have to locate a curve on the crater and then look for  the length of $(\ell+1)$ descending paths on the volcano as described in \cite{volcano}. If there is two paths of length greater than the others then the crater is cyclic, if not the crater is not cyclic. 
\newline 
\todo{detailler ce que fait Miret Moreno}
For $\ell=2$ we can use methods like in \cite{MiretMRV05} which computes an ascending path and can determine the shape of the crater
For $\ell \neq 2$  We can determine an ascending path using methods in \cite{volcano}, the crater wehave a complexity of $O(H^2 \mathcal{F}(\ell) )$, with $H$ the height of the volcano (not known a priori) and $\mathcal{F}(\ell)$ the cost of computing three roots of a modular polynomial. Then we can know the shape of the crater by computing $\ell+1$ descending paths with a complexity of $O(H \mathcal{F}(\ell) \ell)$. %c est exactement ce qui est fait dans ce papier...
...
\item Compute horizontal basis $P,Q$ and $P',Q'$ of
  $E[\ell^k]$ and $E'[\ell^k]$ respectively, for $k > 4r$ \todo{preciser cette borne} ;
