...
contruction of towers of finite fields from~\cite{DoSc12}, in the
...
The interpolation part of the algorithm is slightly different from
what is described in Section~\ref{sec:interpolation}, in that it
handles at once points lying in different orbits of the Frobenius
action: we combine points defined in a same extension of the base
field by Chinese Remaindering, before using the techniques of Section
\ref{sec:interpolation} on all these points at once. Several
quantities used in the Chinese Remainder algorithm are precomputed and
used across subsequent calls to our  interpolation routine, as they
do not depend on the choice of the constants $(a,b)$ appearing in
Step~\ref{alg:ours:for} of the main algorithm.
