...
All the complexity will be expressed in terms of operations in $\mathbb{F}_q$ except when it is stated otherwise.
For $\ell=2$ according to \cite{DoSc12} we have the computation for the Frobenius of an element of $\mathbb{F}_{q^{2^k}}$ done in $O(2^k \log(q))$ operations, the multiplication is done in $M(2^k)$ and the inversion in $O(M(2^k))$ operations in $\mathbb{F}_q$. The square root computation is done in $O(M(2^k)\log(2^kq))$ operations in $\mathbb{F}_q$.
For $\ell \neq 2$  according to \cite{DeDoSc13} the multiplication of an element of $\mathbb{F}_{q^{\ell^k}}$ is done in $M(\ell^k)$ and the inversion in $O(M(\ell^k)\log(\ell^k))$ operations in $\mathbb{F}_q$. For the Frobenius computation we will denote its cost by $F(q,\ell^k)$ operations in $\mathbb{F}_q$.

\subsection*{Notations}

Throughout this paper: $p$~is a prime power, $q$~a power of $p$,
...
\begin{enumerate}
\item If~$π(P) = λ P$ for~$λ ∈ ℤ_ℓ$,
then the isogeny~$ϕ_{ℓ^h P}$ with kernel generated by~$ℓ^h P$ is horizontal.
In this case, we say that $ϕ_{ℓ^h P}$ has \emph{direction} $λ_i$.
...
such that all points of~$E[ℓ^n]$ are rational over~$\mathbb F_{q^{d(n)}}$.
Then:
\begin{enumerate}
...
\section{The action of the Frobenius endomorphism}
...
By Proposition~\ref{prop:degree-l-torsion}~(i),
replacing~$\mathbb F_q$ with an extension of degree up to~$ℓ - 1$,
we also assume that all points of~$E[ℓ]$ are rational over~$\mathbb F_q$.

Our goal here is to explicitly diagonalize the Frobenius endomorphism
in~$E[ℓ^k]$, where $k ≥ h+1$,
in order to identify the directions of the two horizontal isogenies.
By Proposition~\ref{prop:degree-l-torsion}~(ii),
we know that all points of~$E[ℓ^k]$ are rational over~$\mathbb F_{q^{ℓ^k}}$.
Therefore, all our algorithms work in~$\mathbb F_{q^{ℓ^k}}$.
All the complexities of are given as a number of
...
as a product~$ψ \widehat{ψ}$ of an $ℓ$-isogeny and its dual.
...
\STATE $h:=1, k_0:=1$
\FOR {$i=1$  to  $k-1$}
\STATE $P \leftarrow P/\ell$; $Q \leftarrow Q/\ell$.
...
\todo{$k_0$ est-il toujours entier ? et que vaut-il si~$λ_1 = λ_2$ ? (1?)}
\ENDIF
...
\begin{prop}
Algorithm~\ref{Diagonalizedbasis} computes a diagonal basis of the
Frobenius endomorphism with a complexity of at most
% The computation complexity of algorithm \ref{Diagonalizedbasis} is
\begin{equation*}
\begin{array}{ll}
...
label...}
The cost of Algorithm~\ref{Diagonalizedbasis} is dominated
...
%\begin{defi} redondant avec la section 3
...
Before computing such a basis we need the following proposition that tells us how we can compute from an $\ell^i$ (for $i>0$) horizontal isogeny an $\ell^{i+1}$ horizontal isogeny with the use of a diagonalised basis of the $\ell^{h+1}$ torsion with respect to the Frobenius action.

\begin{prop}\label{propcentrale}
Let $E$ be an elliptic curve over a cyclic crater of a volcano of $\ell$-isogenies, we consider a  point of order $\ell^{i} \geqslant \ell$  $Q \in E$ such that $\varphi: E \rightarrow \nicefrac{E}{\langle Q \rangle }$ is a horizontal isogeny of direction $\lambda_2$, $P$ a point of order $\ell^{h+1}$ such that $\pi(P) = \lambda_1P$. We denote by $\phi$ the isogeny: $E\rightarrow E^*=\nicefrac{E}{\langle P \rangle}$. For $R$ a $\ell$ division point of $Q$ we have $\phi(R)$ such that $E^* \rightarrow \nicefrac{E^*}{\langle \phi(R) \rangle}$ is a horizontal isogeny.  
\end{prop}
...
%\newline
%Since the isogeny generated by $Q$ is horizontal and of direction $\lambda_2$ then the $\ell$ isogeny generated by $\ell^{d_Q-1}$ is also horizontal and of direction $\lambda_2$.
 Since $\ell R=Q$ then the $\ell$-isogeny $\psi$ with kernel $\ell^{i}\phi(R)=\phi(\ell^{i-1}Q)$ is the dual isogeny of $\phi$ because it is the one who anhiliates the $\ell$-torsion on $E$ (here $\langle \ell^{h}P, \ell^{i-1}Q \rangle$) together with $\phi$. Thus we have proved that $\psi$ is of direction $\lambda_2$ and horizontal. 
<<<<<<< HEAD
\newline
Since we have $\psi(\phi(R))=Q$, this tells us that the isogeny $\Upsilon$ with kernel $\phi(R)$ is the composition of $\psi$ with the isogeny $\varphi$ of kernel $\langle Q \rangle$. Thus the isogeny $\Upsilon$ is horizontal of degree $\ell^{d_Q+1}$ and of direction $\lambda_2$. 
\end{proof}


%Thus from a basis of the $\ell^{k}$ torsion (for $k > h$) such that each generator is also an eigenvector for the Frobenius we use the previous proposition to increase the degree of horizontal isogenies we know until we know an $\ell^{k}$ horizontal isogeny.
We can now present the following algorithm that computes an horizontal $\ell^k$ isogeny starting from a diagonalized basis of the $\ell^k$ torsion.
...
\ENSURE $P$ a point of $\ell^k$ torsion of $E^{k-1}$ associated to $\lambda_1$ and $L$ a list of $k-1$ horizontal $\ell$-isogenies of direction $\lambda_2$ which goes from $E^{k-1}$ to $E^0$.
...
\begin{proof}
When $\ell=2$ the computations of $\lambda_1P$ and $2^{k-1}P$ takes  $O(M(2^k)k)$ operations in $\mathbb{F}_q$. So the costly part of algorithm \ref{1step} remains the computation of a pre image of multiplication by $\ell$ of a point repeated $k$ times i.e. $O(kM(2^k)\log(2^kq))$. 
\newline
...
>>>>>>> 9f925a077339dd1764cfd6e1ebb0c6ab5d81091e
After the application of algorithm \ref{Diagonalizedbasis}, we use the horizontal path $L$ created of direction $\lambda_2$ by applying it on the point associated to the isogeny of direction $\lambda_1$ to have a generator of an horizontal isogeny of direction $\lambda_1$ and degree $\ell^k$ on the input curve.%mal formule mais a reformuler...
...
<<<<<<< HEAD
In this section we consider the abscissae of points of order $\ell^k$ and generators of horizontal isogenies on the two input curves of problem \ref{prob:isogeny-problem}. We want to compute an interpolation polynomial that interpolates the abscissas of thoses points. We state first a result on the order of the points under the action of the Frobenius.

\begin{prop} %prouve par Jerome dans la section 3 ???
Let $E$ be an elliptic curve defined on $\mathbb{F}_q$ such that $E(\mathbb{F}_q)[\ell^{\infty}]=\mathbb{Z}/\ell^{s+j}\mathbb{Z} \times \mathbb{Z}/\ell^{s}\mathbb{Z}$ with $ j \geqslant 0$ and $\nu_\ell(q-1)>h>1$, then $E(\mathbb{F}_{q^{\ell}})[\ell^{\infty}]  \mathbb{Z}/\ell^{s+j+1}\mathbb{Z} \times \mathbb{Z}/2^{s+1}\mathbb{Z}$
\end{prop}
=======
...
\begin{defi}
Let $E(\mathbb{F}_{q^{\ell^k}})[\ell^{\infty}]=\mathbb{Z}/\ell^{k+j+s} \mathbb{Z} \times \mathbb{Z}/\ell^{k+s} \mathbb{Z}$ with $j \geqslant 1$, $1 \leqslant h \leqslant \nu_\ell(q-1)$, we denote by $o_{\lambda_1}$ and $o_{\lambda2}$ the multiplicative order of the eigenvalues$\lambda_1 , \lambda_2$ of the Frobenius  for the $\ell^{k+h}$ torsion defined on the extension field $\mathbb{F}_{q^{2^k}}$.
\end{defi}

\begin{prop}
$o_A=\max(o_{\lambda_1},o_{\lambda_2}) = \ell^k$ and $o_B=\min(o_{\lambda_1},o_{\lambda_2}) = \ell^{k-j}$
\end{prop}
...
For the rest of this paper we will consider that $s=0$ to be in the worst case.

Since the isogeny we want to compute is rational and we work with points that are defined in extension,
we will work with points that are in different orbits
under the action of the Frobenius. As the size of the orbits are not
always the same, we will have to consider the different possible orders and
combine them efficiently.

\begin{defi}
For $j$ in $[0, \nu_{\ell}(o_A)-\nu_{\ell}(o_B)]$ we denote by $o_j$ the order $o_A \geqslant o_j=\frac{o_A}{\ell^j} \geqslant o_B$.
\end{defi}
...
$\mathcal{T}_j$ and $\mathcal{A}_j$ as follows:
\begin{itemize}
\item $\mathcal{T}_j$ is the monic polynomial whose roots are the
  abscissae of points of $\ell^k$-torsion which generate an orbit of
  order $o_j=\frac{o_A}{\ell^j}$ 
\item $\mathcal{I}_j$ is the Lagrange interpolation polynomial  that interpolates abscissae of points of $\ell^k$-torsion which generate an orbit of order $o_j=\frac{o_A}{\ell^j}$  on abscissae of points with the same property.
\end{itemize}

\begin{prop}
For $j$ in $[0, \nu_{\ell}(o_A)-\nu_{\ell}(o_B)]$ the polynomials $\mathcal{T}_j$ and $\mathcal{I}_j$ are defined over $\mathbb{F}_{q^{o_j}}$
\end{prop}

\begin{proof}
Since $\mathcal{T}_j$ is a polynomial defined with abscissas of points of $\ell^k$-torsion which generate an orbit of order $o_j=\frac{o_A}{\ell^j}$ under the Frobenius action, it is clear that this polynomial is defined in $\mathbb{F}_{q^{o_j}}$ because it is made from points of different orbits of same order $o_j$.
\end{proof}
...
The aim is to get from the different polynomials $\mathcal{T}_j, \mathcal{I}_j$ an interpolation polynomial $A=\prod_{j=0}^{\nu_{\ell}(o_A)-\nu_{\ell}(o_B)}\prod_{i=0}^{\nu_{\ell}(o_A)-j}\pi^{\frac{o_A}{l^i}}(\mathcal{I}_j) \in \mathbb{F}_q[x]$ and $T=\prod_{j=0}^{\nu_{\ell}(o_A)-\nu_{\ell}(o_B)}\prod_{i=0}^{\nu_{\ell}(o_A)-j}\pi^{\frac{o_A}{l^i}}(\mathcal{T}_j) $ using the combined actions of the Frobenius with Chinese Remainder techniques.


%\todo{change the notation $\mathcal{A}_A$, this is not OK. Can replace all 
%$\mathcal{A}$ by $\mathcal{I}$, for instance.} --->done

%\todo{write a lemma somewhere that states in what field these polynomials
%  have their coefficients} --->done
  
  
  
We start by taking into account the greatest orbit with the polynomials $\mathcal{I}_0$, $\mathcal{T}_0 \in \mathbb{F}_{q^{o_A}}[x]$, which has order $o_A$ for the Frobenius action;
we obtain polynomials $R$ and $S$ $\in \mathbb{F}_{q^{\frac{o_A}{\ell}}}[x]$ with a combined action of the Frobenius with Chinese remainder techniques on the polynomials $\mathcal{I}_0$, $\mathcal{T}_0$. Then with $R$ and $S$ of  order $\frac{o_A}{\ell}$ for the Frobenius action we take into account the polynomials of same order for the Frobenius action $\mathcal{I}_1$, $\mathcal{T}_1 \in \mathbb{F}_{q^{\frac{o_A}{\ell}}}[x]$ and we use again a combined action of the Frobenius with Chinese remainder techniques on these polynomials to obtain new polynomials $R$ and $S$ $\in \mathbb{F}_{q^{\frac{o_A}{\ell^2}}}[x]$. We repeat those operations until we get polynomials $R$ and $S$ $\in \mathbb{F}_{q^{\frac{o_B}{2}}}[x]$. From this point we only use a combined action of the Frobenius with Chinese remainder techniques until we get $A$ and $T \in \mathbb{F}_q[x]$.
...
\begin{algorithm}
\caption{\label{computingT2ir} Computing $A,T \in \mathbb{F}_q[x]$} 
\begin{algorithmic}[5]
\REQUIRE All the polynomials $\mathcal{T}_j, \mathcal{I}_j \in \mathbb{F}_{q^{o_j}}$ for $j \in [0, \nu_{\ell}(o_A)-\nu_{\ell}(o_B)]$. 
\ENSURE $T=\prod_{j=0}^{\nu_{\ell}(o_A)-\nu_{\ell}(o_B)}\prod_{i=0}^{\nu_{\ell}(o_A)-j}\pi^{\frac{o_A}{l^i}}(\mathcal{T}_j)$ $A=\prod_{j=0}^{\nu_{\ell}(o_A)-\nu_{\ell}(o_B)}\prod_{i=0}^{\nu_{\ell}(o_A)-j}\pi^{\frac{o_A}{l^i}}(\mathcal{I}_j) \in \mathbb{F}_q[x]$
...
\STATE $\left( R,T \right) \in \mathbb{F}_{q^{o_A/\ell}}[x] \gets CRTs \left( \pi^{o_A/\ell}(\mathcal{I}_A) \bmod  \pi^{o_A/\ell}(\mathcal{T}_A),\mathcal{I}_A \bmod \mathcal{T}_A \right)$
\FOR{$r=1$ to $\nu_\ell(\frac{o_{A}}{o_{B}})$}%\STATE $\left( R_2=\pi(R)^{o_A/2^{1+r}} T_2=\pi(T)^{o_A/2^{1+r}} \right) \gets \left( R,T \right) \in \mathbb{F}_{q^{o_A/(2^r)}}$
\STATE $\left( R , T \right) \in \mathbb{F}_{q^{o_A/(\ell^{r+1})}}[x] \gets CRTs \left( \pi^{o_A/\ell^{1+r}}(R) \bmod \pi^{o_A/\ell^{1+r}}(T), R \bmod T \right) $ 
...
\STATE $\left( B,T_B \right)\in \mathbb{F}_{q^{o_A/(\ell^{r+1})}}[x]  \gets CRT \left( \pi^{o_A/\ell^{1+r}}(\mathcal{I}_r) \bmod \pi^{o_A/\ell^{1+r}} (\mathcal{T}_r),\mathcal{I}_r \bmod \mathcal{T}_r, \right)$ 
\STATE $\left( A,T \right) \gets CRT \left(B \bmod T_B, R \bmod T \right) $
\ENDFOR
\FOR{$r=1$ to $\nu_\ell(o_B)-1$}%\STATE $\left( R_2=\pi(R)^{o_B/2^{1+r}}, T2=\pi(T)^{o_B/2^{1+r}} \right) \gets \left( R,T \right) \in \mathbb{F}_{q^{o_B/(2^r)}} $
\STATE $\left( A , T \right)\in \mathbb{F}_{q^{o_B/(\ell^{r+1})}} \gets CRT \left( R \bmod T ,\pi(R)^{o_B/\ell^{1+r}} \bmod \pi(T)^{o_B/\ell^{1+r}} \right)$
\ENDFOR
\RETURN $A,T$
\end{algorithmic}
\end{algorithm}
...
\todo{Eventually explicit the complexity of computing a shape of the volcano i.e. $\mathcal{F}(\ell)$}
  For the input curves we have to determine if we are on a cyclic crater or a path to an elliptic curve on the crater, to do this we can use methods like in \cite{volcano}. To detect the crater we have a complexity of $O(H^2 \mathcal{F}(\ell) )$, with $H$ the height of the volcano (not known a priori) and $\mathcal{F}(\ell)$ the cost of computing three roots of a modular polynomial. Then we can know the shape of the crater by computing $\ell+1$ descending paths with a complexity of $O(H \mathcal{F}(\ell) \ell)$. %c est exactement ce qui est fait dans ce papier...
 ...
 \begin{enumerate}
\item Compute horizontal basis $P₁,Q_1$ and $P_2,Q_2$ of
  $E_1[\ell^k]$ and $E₂[\ell^k]$ respectively, for $k > 4r$ \todo{preciser cette borne} ;
...
\item\label{alg:modif-couveignes:rational} Deduce a rational fraction
  congruent to $\mathcal{A}$ modulo $E₁[\ell^k]$, and verify that it
  defines an isogeny of degree $r$. If it does, return it, otherwise
  replace $P₂,Q_2$ with an other horizontal basis of $E_2[\ell^k]$ and go back to
  Step~\ref{alg:orig-couveignes:interp}.
\end{enumerate} 
