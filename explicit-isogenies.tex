...
formulas~\cite{velu71} \todo{Proposition de solution kernel vs rational fraction: thus we will consider equivalently the problem of computing the rational fraction $g(x)/h(x)$ that send abscissas of points of one curve on the other and computing the kernel of the isogeny}.
...
is isomorphic to~$(ℤ/ℓ^{n+α} ℤ) × (ℤ/ℓ^{n+β} ℤ)$.\todo{proposition de correction $E[ℓ^{∞}](F_n+1)$
is isomorphic to~$(ℤ/ℓ^{n+α} ℤ) × (ℤ/ℓ^{n+β} ℤ)$, peut etre a-t-elle déjà été supprimé, si c'est le cas ne pas tenir compte}
