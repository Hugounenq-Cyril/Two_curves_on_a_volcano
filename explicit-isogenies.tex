...
%\todo{faire reference au cout de mutliplication des polynômes de l'introduction} 
%the fact that multiplication and Euclidean division of polynomials of degree at most d in $F_i[x]$ can be done in $O(M(d\ell^i))$ operations (as recall in section \ref{sec:introduction}),
...
\newline
\begin{prop}
The points of order $\ell^k$ of $E$ are of order $\ell^{k-\beta} \geqslant o \geqslant \ell^{k-\alpha} $ for the Frobenius action according to the structure of $E[\ell^\infty](\F_k)$.
\end{prop}
...
The cost of computings the different representatives of orbits is majorated by  $O(\ell^{2k}M(\ell^{k-\beta})k \log(\ell))$.
\end{prop}

\begin{proof}
We consider the point of order $\ell^k$ with order $\ell^i$ for the Frobenius, then we have at most $\ell^{2k}$ points of order $\ell^i$, thus overall we have a complexity $\ell^{2k}k\log(\ell) \sum_{i=\beta}^{\alpha}\mathsf{M}(\ell^{k-i})$. Let $c$ be such that we can multiply elements of $\mathbb{F}_{q^{\ell^{k-i}}}$ in $c\ell^{k-i}(k-i)\log(k-i)$ majorated by $c\ell^{k-i}(k-\beta)\log(k-\beta)$, then we get $\sum_{i=\beta}^{\alpha}c\ell^{k-i}(k-\beta)\log(k-\beta) \leqslant c\ell^{k-\beta+1}(k-\beta)\log(k-\beta)$. In the end we get the cost majorated by $c\ell^{k-\beta+1}\ell^{2k}k\log(\ell)(k-\beta)\log(k-\beta)=c\ell^{2k}\mathsf{M}(\ell^{k-\beta})k\log(\ell)$
\end{proof}
