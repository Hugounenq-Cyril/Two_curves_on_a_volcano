...
\begin{proof}
As we consider a set of $\ell^{2k-2}\frac{\ell^2-1}{\ell^2}$ points of order $\ell^k$ with orbits of size going to $\ell^{k-\alpha}$ to $\ell^{k-\beta}$ then we have at most $\ell^{k-2+\beta}\frac{\ell^2-1}{\ell^2}$ representatives of different orbits. For each of them we will work at most in the field $\mathbb{F}_{q^{\ell^{k-\beta}}}$ thus computing a representative will cost us at most $O(M(\ell^{k-\beta})k\log(\ell))$.
\end{proof}
