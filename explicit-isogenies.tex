...
\begin{algorithmic}[1]
\item \textbf{\color{red}CET ALGORITHME EST UNE SUGGESTION DE CORRECTION}
\REQUIRE $E$: an ordinary, $ℓ$-maximal elliptic curve;
$k$: an integer~$≥ h$; $(P_1, Q_1)$: a basis of~$E[ℓ]$.
...
\STATE \textbf{if} $i ≤ h-1$ \textbf{then} $(P_i, Q_i) ← (P', Q')$;
\textbf{next}; \textbf{end if}.
\STATE\label{alg:diagonal:frobenius} compute $\pi|(P',Q')=\left( \begin{smallmatrix}
λ + a\ell^{i} & b\ell^{i}\\
c\ell^{i} & μ + d\ell^{i}
\end{smallmatrix} \right) \pmod {\ell^{i+1}}$
\IF{$i = h$} \STATE{$x ←$ root in~$\F_ℓ$ of $b x^2 + (a-d) x - c = 0$}
\STATE{$P_{i} ← P' + x Q'$; $Q_{i} ← -b/c P' + Q'$}
\STATE{$λ ← λ + ℓ^i (a + b x)$; $μ ← μ + ℓ^i (d - b x)$}
\ELSE \STATE{\textbf{\color{red} ???}}
\ENDIF
\ENDFOR
...
\begin{prop}
  ...
  Assuming that $ℓ\in o(r)$ and $2h<\log_ℓ(r)$, the algorithm outlined
  above \todo{(shall we make a proper float for the algorithm?)}
  computes an isogeny $ψ:E→E'$ using an expected
  \[O\bigl(r\MM(rℓ^2)\log(r)\log(ℓ) + \MM(\sqrt{r}\log(q))\log^2(r) +
    ℓ\MM(ℓ)\log(q)\bigr)\]
  operations in $\F_q$.
\end{prop}
\begin{proof}
  Thanks to the hypotheses, we can pick the integer $k$ so that
  $ℓ^{2k} ∈ O(r)$ on average. \todo{Is this clear enough?}
  ...
  Bounding $h$ by $k-1$, Step~\ref{alg:ours:horizontal} costs
  ...
  $O(r\log(r) +\MM(\sqrt{r}\log(q))\log^2(r) +
  ℓ^2\MM(\sqrt{r})\log(r))$ otherwise.
...
$h$, it has an expected running time which is quasi-quadratic in the isogeny
