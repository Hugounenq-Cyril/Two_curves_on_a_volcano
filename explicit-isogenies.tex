...
\end{figure}
\begin{algorithm}
\caption{\label{alg:horizontal}Computing a horizontal point of order~$ℓ^{h+1+n}$}
% Horizontalisation of $R$ following the $\lambda$ direction}
\begin{algorithmic}[1]
\REQUIRE $(P, Q)$: a diagonal basis of~$E[ℓ^{h+1}]$; $n$: an integer.\\
% $R$: a point of~$E$ of order $ℓ^k$ such that
% $ℓ^n R$~is horizontal of direction~$μ$ for some~$n ≤ k$.
\ENSURE $R$: a horizontal point of~$E[ℓ^{h+1+n}]$ with direction~$λ$.
...
\ENDFOR
\RETURN $R = \widehat{ϕ}_1 ∘ … ∘ \widehat{ϕ}_n ( ℓ^{-n} P_n )$.
