...
\begin{proof}
<<<<<<< HEAD
Since $P$ is horizontal of direction $\lambda$ then by \ref{prop:diagonal-horizontal} there exists a point $R$ of order $\ell^{h+i}$ dividing $P$. As $\psi$ is a $r$ isogeny then $\psi$ doesn't change the order of $P$ and $R$, moreover the frobenius commutes with $\psi$ (because $\psi$ is defined on $\mathbb{F}_q$) then we have $\pi(\psi(R))=\lambda\psi(R)$ we can conclude by \ref{prop:diagonal-horizontal}.
=======
...
>>>>>>> b8ef4e66cb19b8b72998cead0a500981b033009d
\end{proof}
