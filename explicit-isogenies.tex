...
\[O
  \Bigl(\bigl(r ℓ^2\MM(rℓ^4) +  \MM(rℓ^3)\log(ℓq)\bigr)\log(r)\log(ℓ) \Bigr)
...
To validated our findings, we implemented a simplified version of our main algorithm using
SageMath v7.1~\cite{sage}. In our current implementation, we only
handle the case $\ell=2$ and we work only with curves on the
crater of the $2$-volcano.  We implemented the
...
basis of $E[ℓ^k]$, and that for just one execution of the interpolation
