...
The cost of computings the different representatives of orbits is majorated by  $\ell^{\alpha-\beta}O(M(\ell^{2k})k \log(\ell))$.
\end{prop}

\begin{proof}
We consider the point of order $\ell^k$ with order $\ell^i$ for the Frobenius, then we have at most $\ell^{2k}\frac{\ell^i}{\ell^{k-\alpha}}$ points of order $\ell^i$, thus overall we have a complexity $\ell^{k+\alpha}k\log(\ell) \sum_{i=\beta}^{\alpha}\mathsf{M}(\ell^{k-i})$. Let $c$ be such that we can multiply elements of $\mathbb{F}_{q^{\ell^{k-i}}}$ in $c\ell^{k-i}(k-i)\log(k-i)$ majorated by $c\ell^{k-i}(k)\log(k)$, then we get $\sum_{i=\beta}^{\alpha}c\ell^{k-i}(k)\log(k) \leqslant c\ell^{k-\beta+1}(k)\log(k)$. In the end we get the cost majorated by $c\ell^{\alpha-\beta}\ell^{2k}\log(k)k^2\log(\ell)=\ell^{\alpha-\beta}\mathsf{M}(\ell^{2k})k\log(\ell)$
\end{proof}
