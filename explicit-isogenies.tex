...
we easily check that at step~$i$ of the loop,
...
\todo{, with $E[\ell^{k+s}] \subset E(\mathbb{F}_{q^{\ell^k}})$.   What is the last part, ``defined on the
...
We will suppose without loss of generality that $P$ (resp. $Q$) has an eigenvalue $\lambda$ (resp. $\mu$) of order $o_A$ (resp. $o_B$) for the Frobenius, with $(P,Q)$ the horizontal basis computed section \ref{sec:acti-frob-endm}.
\end{defi}
...
%% \end{defi}
For $i$ in $\{0,\dots \nu_{\ell}(o_A)-\nu_{\ell}(o_B)\}$ we define the sets
$\mathcal{I}_i$ as follows:
\newline
For $0 < i < \nu_{\ell}(o_A)-\nu_{\ell}(o_B)$:
$\mathcal{I}_i=\{ \ell^i c P + d Q, c,d \in \left( \mathbb{Z}/ \ell^k \mathbb{Z} \right)^{*} \} $
\newline
For $i= \nu_{\ell}(o_A)-\nu_{\ell}(o_B) $:
$\mathcal{I}_i=\{ \ell^i c P + d Q, c,d \in \left( \mathbb{Z}/ \ell^k \mathbb{Z} \right) \} $ and $d \wedge \ell =1$
\newline
For $i=0$
$\mathcal{I}_0=\{ \ell^i c P + d Q, c,d \in \left( \mathbb{Z}/ \ell^k \mathbb{Z} \right)^{*}$ and $c \wedge \ell =1$
...
$T_i$ and $L_i$ as follows:
\begin{itemize}
\item $\mathcal{T}_i= \prod_{R \in I_i} (x-x(R))$
\item $L_i$ is the Lagrange interpolation polynomial that send: $x(\ell^i c P + d Q)$ on $x(\ell^i c aP' + d b Q')$ for every  $ \ell^i c P + d Q  \in I_i $ and $a,b \in \left( \mathbb{Z}/ \ell^k \mathbb{Z} \right)^{*}$ the parameters for the interpolation mapping.
\end{itemize}
...
\item $\mathcal{T}_i$ is the monic polynomial whose roots are the
  ...
  $\mathcal{T}_i=(\prod(x-x(P)), P \in E ))$
