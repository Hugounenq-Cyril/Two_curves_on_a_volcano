...
$O(ℓ^{4k})$ possible ways, thus yielding an
algorithm of complexity $\tildO(ℓ^{6k})=\tildO(r³)$ at best.
...
dividing $ℓ-1$, and $[F_{i+1}:F_i]=ℓ$ for any $i>0$ \todo{proposition de modification de notation: $\F_q⊂F₀⊂F₁⊂\cdots⊂F_n$ with $\ell$ dividing $\#F_0-1$, $d_0=[F₀:F_q]$
dividing $ℓ-1$, and $[F_{i+1}:F_i]=ℓ$ for any $i \geqslant 0$}. For $ℓ=2$,
...
in which $Φ_R$ has the matrix~$ℓ^k M_R^{-1}$.
...
Then $π|T_ℓ(E')$ has the matrix~$\smat{λ& ℓ^{h-k} x (λ-μ)\\ 0&μ}$.
...
is isomorphic to~$(ℤ/ℓ^{n+α} ℤ) × (ℤ/ℓ^{n+β} ℤ)$. \todo{avec la proposition de notation dans la section 1 c'est bon}
\item The group~$E[ℓ^k]$ contains at most
$k · ℓ^{k+β}$ Galois conjugacy classes over~$F_1 = \F_{q^{d_1}}$. \todo{si on fait le changement de notation, ça devient$F_0 = \F_{q^{d_0}}$}
\end{enumerate}
\end{prop}
\begin{proof}
...
\end{prop}
\begin{proof}
The cost of computing a basis of the $\ell$ torsion is of $O(\mathsf{M}(\ell^2)\log(\ell)\log(\ell^2q))$ dominated by computation of roots of the $\ell$ division polynomial of degree $\ell^2$ using the variant of the Cantor-Zassenhaus algorithm described in \cite[Chapter~14.5]{vzGG}. 
...
\todo{insérer ici la complexité des calculs de 5.1} The cost of computing an horizontal basis of the elliptic curves is of $O(\mathsf{R}(k-\beta)k)$ according to section \ref{sec:acti-frob-endm}, thus for $\ell=2$ we got $O(\mathsf{R}(k-\beta)k) \leqslant O(\mathsf{M}(\ell^{k-\beta})k \log(q)) $, for $\ell \neq 2$ we got $O(\mathsf{R}(k-\beta)k) \leqslant O((\ell^{(k-\beta)(w+1)/2}+\mathsf{M}(\ell^{k-\beta+1}\log(q)))k \log(\ell)) $ \todo{eventuellement epurer les $\beta$}
...
\label{sec:implem}
We implement the algorithm on SageMath, we run the test on SageMath v7.0. We place ourselves in the optimal case of \cite{DoSc12} with $p = 1 \bmod 4$.
