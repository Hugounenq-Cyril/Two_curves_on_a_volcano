...
<<<<<<< HEAD
In this section we consider the abscissae of points of order $\ell^k$ and generators of horizontal isogenies on the two input curves of problem \ref{prob:isogeny-problem} on a cyclic crater of volcano of $\ell$ isogeny. We want to compute an interpolation polynomial that interpolates the abscissas of thoses points. For an horizontal basis $(P_1,Q_1)$ of $E_1$ and $(P_2,Q_2)$ of $E_2$, for this section we consider that the interpolation aim to send $P_1$ on $aP_2$ and $Q_1$ on $bQ_2$ with $a,b \in \left(\mathbb{Z}/\ell^k \mathbb{Z} \right)^*$ 
...
1$, $1 \leqslant h \leqslant \nu_\ell(q-1)$, we denote by
$o_{\lambda_1}$ and $o_{\lambda2}$ the multiplicative order of the
eigenvalues$\lambda_1 , \lambda_2$ of the Frobenius for the
$\ell^{k+h}$ torsion, with $E[\ell^{k+h}] \subset E(\mathbb{F}_{q^{\ell^k}})$.  \todo{What is the last part, ``defined on the
  extension field \dots''? Is it clear? Proved somewhere? Do you need
  it here? --> Corollary of the previous proposition}
\end{defi}

\begin{defi}
We denote by $o_A=\max(o_{\lambda_1},o_{\lambda_2})$ and $o_B=\min(o_{\lambda_1},o_{\lambda_2})$
\end{defi}
...
For the rest of this paper we will consider that $s=0$ to be in the worst case.\todo{What does this mean?}
...
\item $\mathcal{T}_i$ is the monic polynomial whose roots are the
  ...
  order $o_i$ on the domain curve of  problem \ref{prob:isogeny-problem}; 
...
\item $\mathcal{I}_i$ is the Lagrange interpolation polynomial that
  ...
  order $o_j$ on the domain curve of  problem \ref{prob:isogeny-problem} on abscissae of points of the codomain curve with respect to the interpolation coefficients $a$ and $b$. \todo{Does not make much sense. How do you choose which
...
\end{lem}
\begin{proof}
  Since $\mathcal{T}_i$ is a polynomial defined with abscissas of
  points of order $\ell^k$ which are representative of different orbits of order $o_i$
  under the Frobenius action, since the polynomial is made from points of
  different orbits of same order $o_i$ we have the result. We can apply the same reasoning for $\mathcal{I}_i$.\todo{Not clear, I don't understand
...
$$T=\prod_{i=0}^{\nu_{\ell}(o_A)-\nu_{\ell}(o_B)}\prod_{t=0}^{o_i}\pi^{t}(\mathcal{T}_i)$$ 
and
$$I= CRT(\pi^{t}(\mathcal{I}_j) \bmod \pi^{t}(\mathcal{T}_j), i\in \{ 0,.., \nu_{\ell}(o_A)-\nu_{\ell}(o_B) \}  t \in \{ O,..,o_i \} )$$

\todo{Prove that they have coefficients in $\F_q$}
...
\ENSURE $T=\prod_{i=0}^{\nu_{\ell}(o_A)-\nu_{\ell}(o_B)}\prod_{t=0}^{o_i}\pi^{t}(\mathcal{T}_i)$ $I= CRT(\pi^{t}(\mathcal{I}_j) \bmod \pi^{t}(\mathcal{T}_j), i\in \{ 0,.., \nu_{\ell}(o_A)-\nu_{\ell}(o_B) \}  t \in \{ O,..,o_i \} ) \in \mathbb{F}_q[x]$
...
\STATE $\left( I , T \right)\in \mathbb{F}_{q^{o_B/(\ell^{r+1})}} \gets CRT \left( I \bmod T ,\pi(R)^{o_B/\ell^{1+r}} \bmod \pi(T)^{o_B/\ell^{1+r}} \right)$
\ENDFOR
\RETURN $A,T$
\end{algorithmic}
\end{algorithm}

\todo{Choose $x$ or $X$ as a variable once and for all}
...
\begin{lem}\label{lemma:complexity_pol_mul}
  ...
  $\mathbb{F}_{q^{\ell^k}}[X] $ is $O(M(\ell^kn))$ operations in
  $\mathbb{F}_q$.
\end{lem}
...
$O(M(\ell^kn)\log(n))$ operations in $\mathbb{F}_q$.
\end{prop}
