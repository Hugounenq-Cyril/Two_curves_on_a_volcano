...
%In the following subsection the first two points will be taken care of in the choice of the mapping for the interpolation, the third will be taken into account with the construction of the interpolation polynomial.
...
For points of order $\ell^k$ we have a diagonal matrix $D(\lambda, \mu)$ of $\pi|E[\ell^k]$ since the points $R \in E[\ell^]$ are defined in $\mathbb{F}_{q^t}$ when $\pi^t(R)=R$,  with the structure of $E(\mathbb{F}_q)[\ell^{\infty}]$ we have the result.
\end{proof}
...
\mathcal{L}_i$ the polynomials
$$T=\prod_{i=0}^{\nu_{\ell}(o_A)-\nu_{\ell}(o_B)}\prod_{t=1}^{o_i}\sigma^{t}(\mathcal{T}_i)$$ 
and
$$L= CRT(\sigma^{t}(\mathcal{L}_j) \bmod \sigma^{t}(\mathcal{T}_j), i\in \{ 0,.., \nu_{\ell}(o_A)-\nu_{\ell}(o_B) \}  t \in \{ 1,..,o_i \} )$$
with $CRT$ the Chinese Remainder Theorem.

\begin{prop}
$T ,L $ are defined over $\mathbb{F}_q$ and of degree $\frac{\ell^2-1}{2\ell^2}\ell^{2k-2}$. $T$ (resp. $L$) vanishes (resp. interpolates) on all the abscissas of point of order $\ell^k$ of the curve $E$ (resp. of the curve $E$ on $E'$) according to the mapping coefficients $(a,b) \in \left( \mathbb{Z}/ \ell^k \mathbb{Z} ^* \right)^2$.  
\end{prop}
...
\begin{algorithm}
\caption{\label{alg:computingLT} Computing $L,T \in \mathbb{F}_q[x]$} 
\begin{algorithmic}[1]
...
\STATE $\left( L,T \right) \in \mathbb{F}_{q^{o_A/\ell}}[x] \gets CRT \left( \sigma^{t(o_A/\ell)}(\mathcal{L}_A) \bmod  \sigma^{t(o_A/\ell)}(\mathcal{T}_A),t \in \{0, \dots, \ell-1\} \right)$
...
\STATE $\left( L , T \right) \in \mathbb{F}_{q^{o_A/(\ell^{r+1})}}[x] \gets CRT \left( \sigma^{t(o_A/\ell^{1+r})}(L) \bmod \sigma^{t(o_A/\ell^{1+r})}(T), t \in \{0, \dots, \ell-1\} \right) $ 
...
\STATE $\left( B,T_B \right)\in \mathbb{F}_{q^{o_A/\ell^{(r+1)}}}[x]  \gets CRT \left( \sigma^{t(o_A/\ell^{1+r})}(\mathcal{L}_r) \bmod \sigma^{t(o_A/\ell^{1+r})} (\mathcal{T}_r),t \in \{0, \dots, \ell-1\} \right)$ 
...
\STATE $\left( L , T \right)\in \mathbb{F}_{q^{o_B/(\ell^{(r+1)}}} \gets CRT \left( \sigma^{t(o_B/\ell^{1+r})}(L) \bmod \sigma^{t(o_B/\ell^{1+r})}(T), t \in \{0, \dots, \ell-1\} \right)$
\ENDFOR
\RETURN $L,T$
\end{algorithmic}
\end{algorithm}
...
the cost of some basic operations used in it. 
%The first result is
...
%Lemme devenu redondant avecla section 2
...
\begin{lem}
The polynomial $T$ at the step $5$ of the algorithm \ref{alg:computingLT} and at the $r$th iteration is of degree: $ba$. The polynomial $T$ at the step $8$ of the algorithm \ref{alg:computingLT} and at the $r$th iteration is of degree: $ba$.
\end{lem}

\begin{proof}\todo{en cours, donc a finir..}
At the end of step $1$ we got a polynomial $T$ of degree $\frac{\ell^{k+s}}{2}$. The degree of the polynomial $T$ at the step $5$ of the algorithm and at the $r$-th iteration is $\frac{\ell^{k+s-2}}{2}(\ell^{r+2}+\sum_{i=1}^r\ell^i$).
\newline
After $j-1$ iterations of step $2-6$ of algorithm \ref{alg:computingLT} the degree of $T$ is $\frac{\ell^{k+s-2}}{2}(\ell^{j+1}+\frac{\ell^{j}-\ell}{\ell-1})$. After $j$ iterations of step $2-6$ of algorithm \ref{alg:computingLT} the degree of $T$ is $\frac{\ell^{k+s-1}}{2}(\ell^{j+1}+\frac{\ell^{j}-\ell}{\ell-1}+\ell)$
\newline
After $\nu_l(o_B)-1=k-j-s-1$ iterations of step $7-9$ the polynomial $T$ is of degree...
\end{proof}
